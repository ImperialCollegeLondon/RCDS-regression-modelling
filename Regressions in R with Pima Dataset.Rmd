...
plot(pima$bmi, pima$glucose, main = "Scatterplot of BMI and glucose")
```
...
```{r}
# Insert your code here
fit.glucose_bmi <- lm(glucose ~ bmi, data=pima) # fit model
summary(fit.glucose_bmi) # get slope (beta coefficient) and p-values (Pr(>|t|))
confint(fit.glucose_bmi) # get confidence interval
```
...
```{r}
# Insert your code here
fit.all <- glm(test ~ ., family = binomial, pima)
summary(fit.all)
anova(fit.test_glucose.x.bmi, fit.all, test = "LRT")
```
